%% CPD error over (N, k) using multiple Jacobian estimation methods with Monte Carlo
...
% num_trials: number of Monte Carlo trials per (N, k)
% Output:
% error_matrix: [length(Ns) x length(k_vals) x num_methods] CPD errors per method
% error_stds: same size, standard deviations across trials
function [error_matrix, error_stds] = cpd_error_vs_n_k_MC_all(f, vars, r, f_id, k_vals, Ns, num_trials)
...
% one combined plot per k value with all methods
for j = 1:length(k_vals)
    figure;
    hold on;
    for mth = 1:length(methods)
    ...
    filename = sprintf('cpd_error_vs_n_combined_all_k%d_fid_%d.png', k_vals(j), f_id);
