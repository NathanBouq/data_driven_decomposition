%% CPD error vs N and k with Monte Carlo
% f: symbolic function
...
% Output:
% error_matrix: [length(Ns) x length(k_vals)] CPD errors
% error_stds: same size, standard deviations across trials
function [error_matrix, error_stds] = cpd_error_vs_npoints_and_kneighbors_MC(f, vars, r, f_id, k_vals, Ns, num_trials)

f_num = matlabFunction(f, 'Vars', {vars(:)});
m = length(vars);

methods = {@local_jacobian_regression, @(X, Y, k)local_jacobian_ridge(X,Y,k,1e-5), @(X,Y,k)local_poly_regression(X,Y,k,2)};
method_names = {'linear', 'ridge', 'poly'};
...
% One plot per method and per k
for mth = 1:length(methods)
    for j = 1:length(k_vals)
        figure;
        errorbar(Ns, error_matrix(:, j), error_stds(:, j), '-o', 'LineWidth', 2, 'MarkerSize', 8);
        set(gca, 'XScale', 'log', 'YScale', 'log');
        xlabel('Number of points (N)');
        ylabel('Avg CPD Error');
        f_str = strjoin(arrayfun(@char, f, 'UniformOutput', false), '; ');
        title(sprintf('CPD Error vs N (k = %d, method = %s)\nfor f = [%s]', ...
            k_vals(j), method_names{mth}, f_str));
        grid on;
    
        filename = sprintf('cpd_error_vs_n_k%d_method_%s_f_id_%d.png', k_vals(j), method_names{mth}, f_id);
